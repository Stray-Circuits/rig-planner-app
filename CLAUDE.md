...
| Bump release version everywhere | `pnpm version:set <x.y.z>` |
...
**Sheet also has a `footer` slot** for action rows (Cancel / Apply / Delete) that should pin to the bottom of the panel below the scrolling body. The footer carries its own top-border + `env(safe-area-inset-bottom)` padding, so primary modal actions stay visible even when the body content scrolls — don't recreate "actions at the bottom of the scrolling body" patterns; pass them through `footer` instead. `SettingsSheet` is the canonical consumer (PR #104).
...
**Pedal-dim catalog in `src/data/pedalCatalog.ts` is cross-referenced against the PedalPlayground catalog** (`https://raw.githubusercontent.com/PedalPlayground/pedalplayground/master/public/data/pedals.json`, 8425 community-curated rows). Their `Width`/`Height` map 1:1 to our `widthIn`/`depthIn` — Boss DS-1 PP says 2.875 × 5.125; Boss's 73 × 129 mm spec converts to 2.874 × 5.079. When adding a row or correcting one, fetch PP and adopt their value rather than mapping a manufacturer's "Length × Width × Height" yourself: on side-jack pedals (ZVEX Vexter, Empress Reverb/Echosystem, Keeley stack-knob) my reading of the mm spec inverted the axes vs. how the pedal sits on a board, and round 9 of issue #73 (PR #98) had to flip ~9 rows after PP cross-reference. For B's Music Shop cat-art editions and other repaints PP doesn't carry, look up the BASE pedal in PP — repaints share the base enclosure. `findPedalInCatalog` does strict `catalog ⊆ user` token-subset matching with two extra guards: (1) variant-qualifier mismatches reject (user types "polytune deluxe" when catalog has only "PolyTune 3"); (2) size-class fallback returns a same-brand mini-class entry when the exact product isn't carried (typed "TC Electronic Spark Mini" surfaces Hall of Fame Mini's dim).
...
**Release version is one number, sourced from `package.json`.** `tauri.conf.json` carries `"version": "../package.json"` so Tauri reads the field at build time; `src-tauri/Cargo.toml` is kept in sync manually (handled by the bump script). Android `versionCode` is *derived* from versionName in `src-tauri/gen/android/app/build.gradle.kts` via `MAJOR*10000 + MINOR*100 + PATCH` (caps each component at 99 — well past anything we'll ship), so we never hand-manage the integer. The autogenerated `tauri.android.versionCode` in `tauri.properties` is ignored.
...
pnpm android:container:build:release  # also refuses if v<current-version> already tagged
gh release create v1.2.3 --generate-notes
# upload the AAB to Play Console
```

The tag is the single "shipped to Google Play" marker — both `version:set` and `build:release` refuse if `v<x.y.z>` already exists in git. That's enforced because the derived versionCode collides on re-use, and Play Console rejects an upload with a duplicate versionCode anyway; we fail fast instead.
