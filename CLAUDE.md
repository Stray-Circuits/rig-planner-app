...
**Cross-origin isolation is on (COOP + COEP).** `tauri.conf.json` `app.security.headers` and `vite.config.ts` `server.headers` both set `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: credentialless`. This enables `SharedArrayBuffer` so ORT WASM can run multi-threaded — without it the worker silently falls back to single-threaded WASM (you'll see the warning `env.wasm.numThreads is set to N, but this will not work unless you enable crossOriginIsolated mode` in the webview console). **Why `credentialless` and not `require-corp`:** the Brave image-search results render via `<img src={crossorigin-host}>` and those hosts don't send `Cross-Origin-Resource-Policy`. `require-corp` would block them; `credentialless` allows no-cors cross-origin fetches but strips credentials. SAB is still unlocked. Any future cross-origin `<img>` / `<script>` / `<link>` you add will Just Work as long as the host serves it no-cors (it almost always does). If you ever need credentialed cross-origin requests, route them through `@tauri-apps/plugin-http` (which runs in Rust and bypasses isolation entirely).
